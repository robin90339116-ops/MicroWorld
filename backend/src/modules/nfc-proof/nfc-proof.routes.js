...
async function handleNfcProofRoutes({
  ...
  if (req.method === 'POST' && url.pathname === '/api/nfc/sessions') {
    ...
    const result = nfcProofService.createNfcSession(data, auth.account, await readBody(req), options);
  ...
  if (req.method === 'POST' && url.pathname === '/api/nfc/sessions/confirm') {
    ...
    const result = nfcProofService.confirmNfcSession(data, auth.account, await readBody(req), options);
    send(res, result.status, result.body);
    return true;
  }
